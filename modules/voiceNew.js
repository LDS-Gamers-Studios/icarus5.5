...
const actionRow = (options) => {
  ...
  const buttons1 = [
    ...
    // Video/Screen Share Lock/Unlock
...
function getComponents(user, channel, oldMsg) {
  ...
  const embed = u.embed(oldMsg?.embeds[0])
    .setFields([
...
async function edit(int, channel, content) {
  ...
  // Edit the card
  if (content) return int.editReply({ ...getComponents(int.user, channel, int.message), content }).catch(u.noop);
...
function overwrite(channel, perms) {
  let currentOverwrites = channel.permissionOverwrites.cache.map(p => ({
    id: p.id,
  ...
  for (const permission of perms) {
    for (const user of permission.users) {
      ...
      if (current.allow.size === 0 && current.deny.size === 0) {
        currentOverwrites = currentOverwrites.filter(o => o.id !== user);
      }
    }
  }
...
async function selectUsers(int, action) {
  ...
  const menu = new u.SelectMenu.User().setCustomId(id).setMinValues(1).setMaxValues(1).setPlaceholder(`The user to ${action}`);
...
async function lock(int, channel, trying = false) {
  ...
  const newPerms = overwrite(channel, [
    { users: channel.members.map(m => m.id), allow: ["Connect", "SendMessages"] },
    { users: [int.guildId], deny: ["Connect", "SendMessages"] }
  ]);
...
async function updateChannels(oldState, newState, bypass = false) {
  if (oldState && newState) {
    ...
    // delete channel
    if (channel && channel.parentId === u.sf.channels.voiceCategory && channel.id !== u.sf.channels.voiceAFK && oldState.channel.members.size === 0) {
