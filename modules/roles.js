...
async function roleFunc(int, give = true) {
  try {
    ...
    if (p.isAdmin(int.member)) {
      ...
      if (role) {
        if (give) {
          try {
            ...
            return int.reply({ content: `Successfully added the ${role.name} role`, ephemeral: true });
          } catch (e) { return int.reply({ content: `Failed to add the ${role.name} role`, ephemeral: true }); }
        }
        try {
      ...
      return int.reply({ content: `You didn't give me a valid role`, ephemeral: true });
    }
    ...
    if (role) {
      ...
      case "adulting":
        ...
        try {
          ...
          return int.client.getTextChannel(u.sf.channels.modlogs)?.send({ embeds: [embed] });
        } catch (e) { return int.reply({ content: `Failed to take the ${role.name} role`, ephemeral: true }); }
      case "ldsg lady":
        if (give) {
          try {
